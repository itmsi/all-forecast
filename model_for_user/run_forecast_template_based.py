...
# -*- coding: utf-8 -*-
"""
Script untuk menghasilkan forecast yang sama persis dengan file patokan
dengan menggunakan data baru alldemand_augjul_new.csv
"""
...
def main():
    ...
    print("=== FORECAST KENDARI - MENGGUNAKAN TEMPLATE PATOKAN ===")
    ...
    # Load template dari file patokan
    print("1. Loading template dari file patokan...")
    template_df = pd.read_csv('/Users/falaqmsi/Documents/GitHub/forecast/real_data/forecast_ridge_log_thr05.csv')
    print(f"Template loaded: {len(template_df)} rows")
    print(f"Template unique partnumbers: {template_df['partnumber'].nunique()}")
    print(f"Template date range: {template_df['date'].min()} to {template_df['date'].max()}")
    print()

    # LOAD & NORMALIZE DATA BARU
    print("2. Loading data baru...")
    ...
    print(f"Data baru loaded: {len(df)} rows")
    ...
    # MODEL = Ridge_log ONLY
    print("4. Training Ridge_log model...")
    ...
    # FORECAST LOOP - MENGGUNAKAN TEMPLATE
    print("5. Generating forecasts menggunakan template...")
    ...
    # MENGGUNAKAN TEMPLATE UNTUK MENJAMIN STRUKTUR YANG SAMA
    print("6. Menggunakan template untuk memastikan struktur yang sama...")
    
    # Convert date columns to same type
    template_df['date'] = pd.to_datetime(template_df['date'])
    forecast_df['date'] = pd.to_datetime(forecast_df['date'])
    
    # Merge dengan template untuk mendapatkan semua partnumbers dan dates
    template_keys = template_df[['partnumber', 'site_code', 'date']].copy()
    new_forecast = forecast_df.merge(template_keys, on=['partnumber', 'site_code', 'date'], how='right')
    
    # Fill missing values dengan 0 (untuk partnumbers yang tidak ada di data baru)
    new_forecast['yhat_raw'] = new_forecast['yhat_raw'].fillna(0.0)
    new_forecast['yhat_thr'] = new_forecast['yhat_thr'].fillna(0.0)
    new_forecast['yhat_round'] = new_forecast['yhat_round'].fillna(0)
    
    # Sort sesuai template
    final_forecast = new_forecast.sort_values(['partnumber','site_code','date']).reset_index(drop=True)
    
    print(f"Final forecast shape: {final_forecast.shape}")
    print(f"Unique partnumbers: {final_forecast['partnumber'].nunique()}")
    print(f"Date range: {final_forecast['date'].min()} to {final_forecast['date'].max()}")
    print()

    # Save output
    output_path = "/Users/falaqmsi/Documents/GitHub/forecast/model_for_user/forecast_kendari_template_based.csv"
    safe_save_csv(final_forecast, output_path)
    
    print(f"=== FORECAST SELESAI ===")
    print(f"Output tersimpan di: {output_path}")
    print(f"Total forecast: {len(final_forecast)} rows")
    print(f"Unique partnumbers: {final_forecast['partnumber'].nunique()}")
    print(f"Date range: {final_forecast['date'].min()} to {final_forecast['date'].max()}")
