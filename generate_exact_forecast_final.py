#!/usr/bin/env python3
"""
Script untuk memeriksa data yang hilang dan menghasilkan output yang persis sama
"""
...
def generate_exact_forecast_final():
    """Generate forecast yang persis sama dengan notebook menggunakan data yang benar"""
    ...
    print(f"   KENDARI date range: {kendari_data['date'].min()} to {kendari_data['date'].max()}")
    print(f"   KENDARI unique partnumbers: {kendari_data['partnumber'].nunique()}")
    
    # Filter data to end at 2025-07-31 (last day before forecast)
    df = df[df['date'] <= '2025-07-31'].copy()
    
    print(f"   Filtered data shape: {df.shape}")
    print(f"   Filtered date range: {df['date'].min()} to {df['date'].max()}")
    
    # Check KENDARI after filtering
    kendari_filtered = df[df['site_code'] == 'KENDARI']
    print(f"   KENDARI filtered shape: {kendari_filtered.shape}")
    print(f"   KENDARI filtered date range: {kendari_filtered['date'].min()} to {kendari_filtered['date'].max()}")
    print(f"   KENDARI filtered unique partnumbers: {kendari_filtered['partnumber'].nunique()}")
...
if __name__ == "__main__":
    try:
        result = generate_exact_forecast_final()
