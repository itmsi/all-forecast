...
Dengan menggunakan pendekatan yang berbeda untuk mendapatkan semua partnumbers
"""
...
def add_group_lags_rolls_minimal(df, group_cols, target_col='demand_qty'):
    """Add minimal lag features to keep more partnumbers"""
    df = df.sort_values(group_cols + ['date']).reset_index(drop=True)
    g = df.groupby(group_cols, group_keys=False)
    
    # Use only lag_1 to keep more partnumbers
    df['lag_1'] = g[target_col].shift(1)
    
    # Add rolling mean only for partnumbers with enough data
    df['rollmean_7'] = g[target_col].shift(1).rolling(7).mean()
    
    return df

def one_day_forecast_all_parts(history_df, df_sites, fdate, best_est, group_cols, ZERO_THR):
    """Generate forecast for one day including all partnumbers"""
    hist = add_calendar_features(history_df)
    hist = add_group_lags_rolls_minimal(hist, group_cols, target_col='demand_qty')
    
    # Get latest features for partnumbers that have them
    ...
    # Create forecast combinations for ALL partnumbers in df_sites
    ...
    # Fill missing lag/rolling features with 0
    lagroll_cols = [c for c in hist.columns if c.startswith('lag_') or c.startswith('rollmean_')]
    ...
    feature_cols_num = ['year','month','day','dayofweek','weekofyear','is_month_start','is_month_end'] + lagroll_cols
...
def generate_exact_forecast_final_approach():
    """Generate forecast yang persis sama dengan notebook menggunakan pendekatan final"""
    
    print("=== GENERATE FORECAST PERSIS SAMA DENGAN NOTEBOOK (FINAL APPROACH) ===\n")
    ...
    kendari_data = df[df['site_code'] == 'KENDARI']
    print(f"   KENDARI data shape: {kendari_data.shape}")
    print(f"   KENDARI unique partnumbers: {kendari_data['partnumber'].nunique()}")
    ...
    # Feature engineering with minimal lags
    print(f"\n⚙️  Feature engineering (minimal lags)...")
    ...
    df_fe = add_group_lags_rolls_minimal(df_fe, group_cols, target_col='demand_qty')
    
    # Fill NaN values with 0
    df_fe['lag_1'] = df_fe['lag_1'].fillna(0)
    df_fe['rollmean_7'] = df_fe['rollmean_7'].fillna(0)
    ...
    # Split train/valid
    feature_cols_cat = ['partnumber','site_code']
    feature_cols_num = ['year','month','day','dayofweek','weekofyear','is_month_start','is_month_end','lag_1','rollmean_7']
    
    cutoff = df_fe['date'].max() - pd.Timedelta(days=max(7, 2*FORECAST_HORIZON))
    ...
    # Create ALL KENDARI partnumbers from original data
    all_kendari_parts = kendari_data['partnumber'].unique()
    df_sites = pd.DataFrame({
        'partnumber': all_kendari_parts,
        'site_code': 'KENDARI'
    })
    ...
    for h in range(FORECAST_HORIZON):
        ...
        day_fc = one_day_forecast_all_parts(hist_all, df_sites, fdate, best_est, group_cols, ZERO_THR)
    ...
    # Save output
    output_file = "/app/forecast_exact_final_approach.csv"
...
if __name__ == "__main__":
    try:
        result = generate_exact_forecast_final_approach()
